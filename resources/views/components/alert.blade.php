<!-- Alert Component -->
@props(['type' => 'info', 'title' => '', 'closeable' => false, 'icon' => true])
...
<div class="rounded-lg border {{ $typeConfig['border'] }} {{ $typeConfig['bg'] }} p-4" x-data="{ show: true }"
    x-show="show">
    <div class="flex items-start">
        @if ($icon)
            <div class="flex-shrink-0 {{ $typeConfig['text'] }}">
                {!! $typeConfig['icon'] !!}
            </div>
        @endif
        <div class="ml-3 flex-1">
            @if ($title)
                <h3 class="text-sm font-medium {{ $typeConfig['text'] }}">{{ $title }}</h3>
            @endif
            <div class="text-sm {{ $typeConfig['text'] }} mt-1">
                {{ $slot }}
            </div>
        </div>
        @if ($closeable)
            <div class="ml-3 flex-shrink-0">
                <button @click="show = false" class="{{ $typeConfig['text'] }} hover:opacity-70 transition">
