...
<div class="grid gap-4 md:grid-cols-2">
    ...
    <div>
        ...
        <x-text-input id="kod" name="kod" type="text" class="mt-1 block w-full"
            :value="old('kod', $sumberHasilModel->kod ?? '')" required />
    ...
    <div>
        ...
        <x-text-input id="jenis" name="jenis" type="text" class="mt-1 block w-full"
            :value="old('jenis', $sumberHasilModel->jenis ?? '')" placeholder="Contoh: Derma Jumaat" required />
        <x-input-error :messages="$errors->get('jenis')" class="mt-2" />
    </div>

    <div class="md:col-span-2">
        <x-input-label for="nama_sumber" :value="__('Nama Sumber Hasil')" />
        <x-text-input id="nama_sumber" name="nama_sumber" type="text" class="mt-1 block w-full"
            :value="old('nama_sumber', $sumberHasilModel->nama_sumber ?? '')" placeholder="Contoh: Sumbangan Online" required />
    ...
    <div>
        ...
        <select id="aktif" name="aktif"
            ...
            <option value="1" @selected((bool) old('aktif', $sumberHasilModel->aktif ?? true) === true)>Aktif</option>
            <option value="0" @selected((bool) old('aktif', $sumberHasilModel->aktif ?? true) === false)>Tidak aktif</option>
        </select>
