<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Pengurusan Sumber Hasil</h2>
                <p class="mt-1 text-sm text-gray-500">Urus sumber hasil seperti Derma Jumaat, Sumbangan Online, dan Sewaan.</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-sm font-medium text-slate-500">Jumlah Sumber</p>
            ...
            <form method="GET" class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_180px_auto_auto]">
                    <x-text-input name="q" type="text" class="block w-full" placeholder="Cari kod, nama, jenis"
                        :value="$search" />
                    <select name="jenis"
                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="">Semua jenis</option>
                        @foreach ($jenisOptions as $option)
                            <option value="{{ $option }}" @selected($jenis === $option)>{{ $option }}</option>
                        @endforeach
                    </select>
                    <select name="status"
                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Kod</th>
                            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Nama Sumber</th>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse ($sumberHasil as $item)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right text-sm space-x-3">
                                    @can('update', $item)
                                        <a href="{{ route('admin.sumber-hasil.edit', $item) }}" class="text-indigo-600 hover:text-indigo-900">Ubah</a>
                                    @endcan
                                    @can('toggleStatus', $item)
                                        <form method="POST" action="{{ route('admin.sumber-hasil.status', $item) }}" class="inline">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    @endcan
                                    @can('delete', $item)
                                        <form method="POST" action="{{ route('admin.sumber-hasil.destroy', $item) }}" class="inline" onsubmit="return confirm('Padam sumber hasil ini?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-6 text-center text-sm text-gray-500">Tiada sumber hasil dijumpai.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
