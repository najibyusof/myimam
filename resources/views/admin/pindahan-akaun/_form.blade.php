...
<div class="space-y-5">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div>
            <label for="tarikh" class="block text-sm font-medium text-gray-700 mb-1">
                Tarikh <span class="text-rose-500">*</span>
            </label>
        ...
        <div>
            <label for="amaun" class="block text-sm font-medium text-gray-700 mb-1">
                Amaun (RM) <span class="text-rose-500">*</span>
            </label>
            <input type="number" id="amaun" name="amaun" step="0.01" min="0.01"
                   value="{{ old('amaun', $record?->amaun) }}"
                   placeholder="0.00"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div>
            <label for="dari_akaun_id" class="block text-sm font-medium text-gray-700 mb-1">
                Dari Akaun <span class="text-rose-500">*</span>
            </label>
            <select id="dari_akaun_id" name="dari_akaun_id"
                ...
                <option value="">-- Pilih Akaun Sumber --</option>
                @foreach ($akaunOptions as $akaun)
                    <option value="{{ $akaun->id }}"
                        @selected(old('dari_akaun_id', $record?->dari_akaun_id) == $akaun->id)>
                        {{ $akaun->nama_akaun }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            <label for="ke_akaun_id" class="block text-sm font-medium text-gray-700 mb-1">
                Ke Akaun <span class="text-rose-500">*</span>
            </label>
            <select id="ke_akaun_id" name="ke_akaun_id"
                ...
                <option value="">-- Pilih Akaun Tujuan --</option>
                @foreach ($akaunOptions as $akaun)
                    <option value="{{ $akaun->id }}"
                        @selected(old('ke_akaun_id', $record?->ke_akaun_id) == $akaun->id)>
                        {{ $akaun->nama_akaun }}
                    </option>
                @endforeach
            </select>
    ...
    <div>
        ...
        <textarea id="catatan" name="catatan" rows="3"
                  ...
                  placeholder="Catatan tambahan (pilihan)">{{ old('catatan', $record?->catatan) }}</textarea>
        @error('catatan')
            <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
        @enderror
    </div>
