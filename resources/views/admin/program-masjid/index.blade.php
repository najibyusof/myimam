<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Pengurusan Program Masjid</h2>
                <p class="mt-1 text-sm text-gray-500">Urus program seperti kuliah, program Ramadan, dan aktiviti khas
                    lain untuk dipautkan pada transaksi hasil atau belanja.</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto space-y-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-sm font-medium text-slate-500">Jumlah Program</p>
                ...
                <div class="rounded-3xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
                    <p class="text-sm font-medium text-amber-700">Dipaut Pada Transaksi</p>
            ...
            <form method="GET" class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_auto_auto]">
                    <x-text-input name="q" type="text" class="block w-full" placeholder="Cari nama program"
                        :value="$search" />
                    <select name="status"
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse ($programMasjid as $item)
                            ...
                            <tr>
                                <td class="px-4 py-3 text-sm font-medium text-gray-900">
                                    ...
                                    @if ($usageCount > 0)
                                        <p class="mt-1 text-xs text-amber-700">Dipaut dalam {{ $usageCount }}
                                            transaksi</p>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 text-right text-sm space-x-3">
                                    ...
                                    @can('delete', $item)
                                        <form method="POST" action="{{ route('admin.program-masjid.destroy', $item) }}"
                                            class="inline" onsubmit="return confirm('Padam program masjid ini?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-6 text-center text-sm text-gray-500">Tiada program
                                    masjid dijumpai.</td>
                            </tr>
                        @endforelse
                    </tbody>
