<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('akaun.edit_title') }}</h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 bg-slate-50 px-6 py-4">
                    ...
                    <p class="mt-1 text-sm text-slate-600">{{ __('akaun.panel.edit_subtitle') }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('admin.akaun.update', $akaun) }}" class="space-y-6">
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 px-6 py-4">
                        ...
                        <x-primary-button>{{ __('akaun.form.save_changes') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
