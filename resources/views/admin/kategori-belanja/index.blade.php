<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Pengurusan Kategori Belanja</h2>
                <p class="mt-1 text-sm text-gray-500">Klasifikasi belanja seperti Utiliti, Penyelenggaraan, dan Gaji.</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-sm font-medium text-slate-500">Jumlah Kategori</p>
            ...
            <form method="GET" class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_auto_auto]">
                    <x-text-input name="q" type="text" class="block w-full"
                        placeholder="Cari kod atau nama kategori" :value="$search" />
                    <select name="status"
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                                ...
                                <td class="px-4 py-3 text-right text-sm space-x-3">
                                    ...
                                    @can('delete', $item)
                                        <form method="POST" action="{{ route('admin.kategori-belanja.destroy', $item) }}"
                                            class="inline" onsubmit="return confirm('Padam kategori ini?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500">Tiada kategori
                                    belanja dijumpai.</td>
                            </tr>
                        @endforelse
                    </tbody>
